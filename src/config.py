...
class SAGEConfig:
    ...
    api_key: str = "NULL"
    data_dir: Path = Path("./GRAPHDATA")
    data_dir_out: Path = Path("./GRAPHDATA_OUTPUT")
    embedding_model_path: Path = Path("./models/SEMIKONG-8b-GPTQ")
    embedding_file: str = "SEMIKONG.pkl"
    chroma_dir: Path = Path("./chroma")
    chroma_collection: str = "KG_SEMIKONG"
    cache_seed: int = 9527
    ...
    def from_env(cls) -> "SAGEConfig":
        return cls(
            ...
            data_dir=Path(os.getenv("QA_DATA_DIR", "./GRAPHDATA")),
            data_dir_out=Path(os.getenv("QA_DATA_DIR_OUT", "./GRAPHDATA_OUTPUT")),
            embedding_model_path=Path(os.getenv("QA_EMB_MODEL_PATH", "./models/SEMIKONG-8b-GPTQ")),
            embedding_file=os.getenv("QA_EMBEDDING_FILE", "SEMIKONG.pkl"),
            chroma_dir=Path(os.getenv("QA_CHROMA_DIR", "./chroma")),
            chroma_collection=os.getenv("QA_CHROMA_COLLECTION", "KG_SEMIKONG"),
